...
<img width="1143" height="1028" alt="Screenshot 2025-10-09 101225" src="https://github.com/user-attachments/assets/e02da306-0eed-41cf-b5bf-4629f2e273a5" />

 
## คำถามการทดลอง
ให้จับคู่ Code ส่วนของการทดสอบ กับ Code การทำงาน มาอย่างน้อย 3 ฟังก์ชัน พร้อมอธิบายการทำงานของแต่ละกรณี
```plaintext
1. ฟังก์ชัน add_task() (อยู่ใน app/routes.py)
การทำงาน : ฟังก์ชันนี้เอาไว้รับข้อมูลงานใหม่จากผู้ใช้ (เช่น title และ description) ผ่าน POST request จากนั้นมันจะสร้าง task ใหม่ แล้วบันทึกลงฐานข้อมูล
สุดท้ายจะส่งข้อมูล task ที่สร้างเสร็จกลับไปให้ผู้ใช้ พร้อม status code 201 (หมายถึงสร้างข้อมูลสำเร็จ)
การทดสอบ : โค้ดทดสอบจะลองส่ง POST ไปที่ /tasks พร้อมข้อมูลตัวอย่าง แล้วตรวจสอบว่าได้ status code 201 กลับมาและข้อมูลที่ได้ (เช่น title และ description) ตรงกับที่ส่งไป

2. ฟังก์ชัน get_tasks() (อยู่ใน app/routes.py)
การทำงาน : ฟังก์ชันนี้เอาไว้ดึง รายการงานทั้งหมด ที่อยู่ในฐานข้อมูลแล้วส่งกลับไปในรูปแบบ JSON เป็น list (เหมือนลิสต์งานหลาย ๆ อัน)
การทดสอบ : โค้ดทดสอบจะส่ง GET ไปที่ /tasks ตรวจสอบว่าได้ status code 200 (หมายถึงเรียกสำเร็จ) และข้อมูลที่กลับมามีจำนวนงานตรงกับที่มีในฐานข้อมูล

3. ฟังก์ชัน create_app() (อยู่ใน app/__init__.py)
การทำงาน : ฟังก์ชันนี้ใช้สำหรับสร้างแอป Flask หลัก มันจะตั้งค่าแอป, เชื่อมต่อฐานข้อมูล, และเพิ่ม routes (เส้นทาง API) เข้าไป
เป็นเหมือน “ตัวเริ่มต้น” ของโปรเจกต์ Flask
การทดสอบ: โค้ดทดสอบจะลองเรียก create_app() เพื่อดูว่าสร้างแอปได้จริงไหม แล้วลองเข้า endpoint เบื้องต้น เช่น / หรือ /tasks
...
1. **Docker Architecture**:
   - เหตุใดจึงต้องแยก database และ application เป็นคนละ containers ?
      เพื่อให้ขยายระบบได้ง่ายและอิสระต่อกัน การแยก Container ทำให้เราสามารถจัดสรรทรัพยากร (CPU, RAM) ให้เหมาะสมกับภาระงานของแต่ละส่วนได้อย่างมีประสิทธิภาพ

   - Multi-stage build มีประโยชน์อย่างไร?
      ช่วยทำให้ Docker Image (ไฟล์โปรแกรม) สุดท้าย มีขนาดเล็กและปลอดภัยขึ้น โดยทิ้งเครื่องมือที่ไม่จำเป็นออกไป เหลือไว้แค่โค้ดที่ต้องใช้งานจริง Dockerfile สะอาดและจัดการง่าย

2. **Testing Strategy**:
   - การวัด code coverage มีความสำคัญอย่างไร?
      เครื่องมือที่ช่วยชี้ให้รู้ว่าโค้ดส่วนไหนที่ยังไม่ได้ถูกเทสเพื่อให้เราไปหาจุดบอดที่อาจมีบั๊กซ่อนอยู่ และลดความเสี่ยงของโปรแกรม
เป็นเครื่องมือที่มีประโยชน์มาก ในการค้นหาความเสี่ยงและส่วนที่ยังไม่ได้รับการทดสอบใน codebase

3. **Deployment**:
   - Health check endpoint มีความสำคัญอย่างไร?
      เปรียบเสมือนระบบที่คอยตรวจสุขภาพแอปของเราตลอดเวลา ถ้าพบว่าแอปป่วยหรือไม่ตอบสนอง ระบบจะสร้างตัวใหม่ขึ้นมาแทนให้เองอัตโนมัติทำให้เว็บไม่ล่ม

   - Render และ Railway มีความแตกต่างกันอย่างไร?
      Render: เหมือนคู่มือที่ชัดเจน เหมาะกับคนที่อยากได้ความยืดหยุ่นและความเสถียร ซึ่งปรับแต่งได้ละเอียด และใช้งานระยะยาว
      Railway: เน้นความเร็วและอัตโนมัติ เน้นความง่าย ไม่ต้องตั้งค่าอะไรมาก ระบบจะตรวจจับ framework ให้อัตโนมัติ
